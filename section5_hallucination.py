...
vectorstore = Chroma.from_texts(
    ...
    collection_name="support_docs_hallucination"
)
...
def detect_hallucination_markers(response_text):
    """Simple heuristic checks for potential hallucination"""
    hallucination_phrases = [
        "I believe", "probably", "might be", "I think",
        "generally", "typically", "usually", "in most cases"
    ]

    found_markers = [phrase for phrase in hallucination_phrases
                     if phrase.lower() in response_text.lower()]

    if found_markers:
        print(f"[WARN] Hedging language detected: {found_markers}")
        print("This might indicate the model is uncertain or hallucinating")
    else:
        print("[OK] No obvious hedging language")

def detect_contradictions(response_text, retrieved_context):
    """Check if response contradicts the context"""
    prompt = f"""Retrieved context: {retrieved_context}
...
print("\nHallucination check:")
detect_hallucination_markers(response.content)

print("\nContradiction check:")
context = "\n".join([d.page_content for d in docs])
contradiction_check = detect_contradictions(response.content, context)
